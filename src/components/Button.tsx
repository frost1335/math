export default function Button({ children, disabled = false, className, onClick, type = 'button', status = 'primary' }: {
    ...
    return (
        <button disabled={disabled} type={type} onClick={onClick} className={`${className} ${theme} px-4 md:px-8 py-3 tracking-wide font-medium text-white rounded-tr-2xl border-2 rounded-bl-2xl shadow-lg transition-all disabled:hover:text-white disabled:opacity-50`}>
            {children}
        </button>
    )
};
